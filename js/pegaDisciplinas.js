...
function selecionaDisciplina()
{
    $("#selectDisciplina").on("change", function(){
        ...
        console.log(disciplinaNume);
        console.log(disciplinaNome);
    })
}
...
{
    $("#selecionaDisciplina").on("keyup", function(){
        var disciplinaDigitada = $(this).val();
        if (disciplinaDigitada.length > 2){
            $.ajax({
                async: false,
                url: 'DAO/banco-disciplina-post.php',
                method: 'post',
                data: {disciplina:disciplinaDigitada},
